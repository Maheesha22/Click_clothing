...
export default function UploadSlipPage() {
  ...
  const verifyOrder = async (oNum, email) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3000/api/orders/search/verify?orderNumber=${encodeURIComponent(oNum)}&email=${encodeURIComponent(email)}`
      );
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await fetch(`http://localhost:3000/api/orders/${order.id}/upload-slip`, {
        method: 'POST',
        body: fd,
      });
      const data = await res.json();
