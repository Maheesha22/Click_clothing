...
export default function BankSlips() {
  ...
  const handleVerify = async (orderId, newStatus) => {
    setUpdating(true);
    try {
      await axios.put(`http://localhost:3000/api/orders/${orderId}/payment`, {
            ...
            <tbody>
              {loading ? (
                ...
                const slipUrl = o.payment_slip
                  ? (o.payment_slip.startsWith('http') ? o.payment_slip : `http://localhost:3000/uploads/${o.payment_slip}`)
                  : null;
      ...
      <Modal open={!!viewOrder} onClose={() => setViewOrder(null)} title={null}>
        {viewOrder && (() => {
          const slipUrl = viewOrder.payment_slip
            ? (viewOrder.payment_slip.startsWith('http') ? viewOrder.payment_slip : `http://localhost:3000/uploads/${viewOrder.payment_slip}`)
            : null;
