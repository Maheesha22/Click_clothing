...
function YouMayAlsoLike({ onProductClick, excludeProductId = null }) {
  ...
  useEffect(() => {
    const fetchRecommendations = async () => {
      try {
        const response = await fetch(apiUrl('/you-may-also-like'));
...
export default function Home() {
  ...
  useEffect(() => {
    const fetchLatestProducts = async () => {
      try {
        const response = await fetch(apiUrl('/category-data/latest-products'));
        const data = await response.json();
  ...
  useEffect(() => {
    const fetchBestSellers = async () => {
      try {
        const response = await fetch(apiUrl('/category-data/best-sellers'));
        const data = await response.json();
        if (data.success && Array.isArray(data.data)) {
